...
- ⚙️ **设置界面**: 顶栏齿轮按钮可切换主题（系统/明亮/暗黑，跟随系统时自动响应系统外观）和「别名」列显示开关，设置持久化保存
...
桌面客户端顶部有 **Web 界面** 按钮，点击后会在系统浏览器中打开同一个界面，方便分享给不想安装客户端的用户。可在设置（顶栏齿轮）中关闭「开启 Web 界面」，关闭后按钮消失且浏览器无法访问界面。

### 操作流程

1. 点击 **「选择目录」** 选择要扫描的文件夹
2. 程序自动扫描目录下所有 `xlsx` / `xlsm` / `xls` 文件并建立索引，新文件会自动提取单元格内容（即自动深度索引），**单元格搜索开箱即用**；「深度索引」按钮可手动补索，无变化的重新扫描不会重复深索
3. 在搜索框输入关键词进行搜索：
   - **子表名称**: 搜索 Sheet 名称
   - **文件名**: 搜索文件名
   - **单元格**: 搜索实际的单元格数值
