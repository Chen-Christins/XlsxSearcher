...
### Added
- **系统托盘常驻**：关闭窗口不再直接退出，而是隐藏到系统托盘（macOS 为菜单栏图标）。托盘右键菜单提供“显示主窗口”“关闭时最小化到托盘”（可勾选并持久化到 `QSettings`）“关于 XlsxSearcher <版本>”与“退出”；首次隐藏提示一次，macOS 点击 Dock 图标可恢复窗口；系统无托盘时自动退回原有关闭即退出行为
...
### Fixed
- **Windows 查看版本号**：托盘右键菜单新增“关于 XlsxSearcher”入口，修复 Windows 上除 F1 外无法查看版本号的问题；窗口隐藏到托盘时关于弹窗也能正常置顶获得焦点
- **Windows onedir 打包**：`--name XlsxSearcher.exe` 会使 onedir 产物目录变成 `XlsxSearcher.exe`，导致 CI 打包失败；改用 `--name XlsxSearcher`，三平台产物目录命名统一
