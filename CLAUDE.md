...
- **Settings**: the top bar gear button opens a settings panel (theme System/Light/Dark, resolved to `data-theme` on `<html>`, plus the optional "Alias" column toggle and a "Web 界面" enable switch). Settings persist via `POST /api/settings` into `webui_state.json` (`app_state.rs::set_settings`), and are served back through `/api/state`; `localStorage` is not used because the local server port changes every launch.
- **Web interface token**: `main.rs` generates a per-launch `web_token` (AppState) and opens the webview at `http://127.0.0.1:<port>/?token=<token>`. A middleware (`require_token` in `server.rs`) rejects every request except `/api/health` unless the query token or the `xlsx_token` cookie matches; the first valid UI load sets the cookie. This keeps the desktop webview working while blocking casual browser access. When the `enable_web_interface` setting is off, `POST /api/open-browser` returns "Web 界面已关闭" and the frontend hides the "Web 界面" button.
